...
class VendorMultiPayment(models.Model):
    ...
    name = fields.Char("Name", default="Draft Payment", tracking=True)
    partner_id = fields.Many2one('res.partner', string="Partner", domain=[('supplier_rank', '>', 0)], required=True, tracking=True)
...
#     currency_rate = fields.Float('Currency Rate', tracking=True)
    payment_difference = fields.Float(string="Payment Difference", copy=False, store=True, compute='_compute_amount', readonly=True, tracking=True)
...
#                 raise UserError(_("Please add some amount to Debit lines"))
        ...
        if self.credit_line_ids or self.debit_line_ids:
            if self.paid_amount == 0.00:
                for line in self.debit_line_ids:    
                    if line.move_id:                
                        move_line_invoice_id = self.env['account.move.line'].search([('move_id', '=', line.move_id.id), ('reconciled', '=', False), ('credit', '>', 0)])
                    ...
                    for lines in self.credit_line_ids:
                        if line.amount_reconcile != 0.0:
                            if lines.move_id:
                                move_line_re_invoice_id = self.env['account.move.line'].search([('move_id', '=', lines.move_id.id), ('reconciled', '=', False), ('debit', '>', 0)])
                ...
                if self.payment_difference > 0.00:
                    ...
                    if not debit_line_id:
                        raise UserError(_("Payment difference exceeds Debit lines amount to pay"))
                    move_line_invoice_id = self.env['account.move.line'].search([('move_id', '=', debit_line_id.move_id.id), ('reconciled', '=', False), ('credit', '>', 0)])
                    vals={
