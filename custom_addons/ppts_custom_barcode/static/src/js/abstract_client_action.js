...
var ClientAction = AbstractAction.extend({
    ...
    init: function (parent, action) {
        ...
        this.commands = {
            'O-CMD.LOAD-UNLOAD-CONTAINERS': this._load_unload_containers.bind(this),
            'O-CMD.CREATE-CONTAINERS': this._create_containers.bind(this),
            'O-CMD.CREATE-ACTION-PLAN': this._create_action_plan.bind(this),
            'O-CMD.OPEN-CONTAINERS': this._open_containers.bind(this),
            'O-CMD.CLOSE-CONTAINERS': this._close_containers.bind(this),
            'O-CMD.NON-CONFORMITY-CONTAINERS': this._non_conformity_containers.bind(this),
    ...
    *
    ...
    _load_unload_containers: function () {
        var self = this;
        self._save().then(function () {
            self.do_action('ppts_inventory_customization.action_production_load_unload', {
                clear_breadcrumbs: true,
            });
        });
    },  

    _create_containers: function () {
        var self = this;
        self._save().then(function () {
            self.do_action('ppts_inventory_customization.create_container_action', {
                clear_breadcrumbs: true,
            });
        });
    },

    _create_action_plan: function () {
        var self = this;
        self._save().then(function () {
            self.do_action('ppts_inventory_customization.action_create_action_container', {
                clear_breadcrumbs: true,
            });
        });
    },

    _open_containers: function () {
        var self = this;
        self._save().then(function () {
            self.do_action('ppts_inventory_customization.action_open_containers_menu', {
                clear_breadcrumbs: true,
            });
        });
    },

    _close_containers: function () {
        var self = this;
        self._save().then(function () {
            self.do_action('ppts_inventory_customization.action_open_containers_menu', {
                clear_breadcrumbs: true,
    ...
    _non_conformity_containers: function () {
        var self = this;
        self._save().then(function () {
            self.do_action('ppts_inventory_customization.action_non_confirmed_containers', {
                clear_breadcrumbs: true,
            });
        });
    },
