...
class StockBarcodeController(http.Controller):
    ...
    def main_menu(self, barcode, **kw):
        ...
        ret_container_actions = self.try_container_actions(barcode)
        if ret_container_actions:
            return ret_container_actions
    ...
    def try_container_actions(self, barcode):
        if barcode == 'Create Container':
            view_id = request.env.ref('ppts_inventory_customization.create_container_wizard_form').id
            return {
                'action': {
        ...
        elif barcode == 'Load/Unload Truck':
            view_id = request.env.ref('ppts_custom_barcode.custom_mobile_view_tree_form').id            
            stock_picking_obj = request.env['stock.picking'].search([('state', '=', 'load_unload')])
            
            create_tree = request.env['custom.mobile.view.tree'].sudo().create(
            {
                'name':'Load/Unload Truck',
                'stock_picking_ids':stock_picking_obj,
                'is_stock_picking':True,
            })

            return {
                'action': {
                    'name': 'Load/Unload Truck',
                    'res_model': 'custom.mobile.view.tree',
                    'view_mode': 'form',
                    'views': [(view_id, 'form')],
                    'type': 'ir.actions.act_window',
                    'res_id': create_tree.id,
                    'target': 'current',
                }
            }
           
        elif barcode == 'Create Action Plan':
            
            container_obj = request.env['project.container'].search([('state', '=', 'confirmed')])
            ...
            create_tree = request.env['custom.mobile.view.tree'].sudo().create(
            {
                ...
                'project_container_ids':container_obj,
        ...
        elif barcode == 'Open Containers':
           
            container_obj = request.env['project.container'].search([('state', '=', 'planned')])
        ...
        elif barcode == 'Close Containers':
            
            container_obj = request.env['project.container'].search([('state', '=', 'in_progress')])
        ...
        elif barcode == 'Non Confirmed Containers':
            
            container_obj = request.env['project.container'].search([('state', '=', 'non_conformity')])
            
            view_id = request.env.ref('ppts_custom_barcode.custom_mobile_view_tree_form').id            
                       
            create_tree = request.env['custom.mobile.view.tree'].sudo().create(
            {
                'name':'Non Confirmed Containers',
                'project_container_ids':container_obj,
                'is_project_container':True,
            })
    ...
    def try_open_picking(self, barcode):
        ...
        if corresponding_picking:
            return self.with_context({'barcode':barcode}).get_action(corresponding_picking.id)
        return False
    ...
    def try_new_internal_picking(self, barcode):
        ...
        if corresponding_location:
            ...
            if internal_picking_type:
                ...
                return self.with_context({'barcode':barcode}).get_action(picking.id)
            else:
